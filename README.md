...
Core XNAT includes a DICOM SCP receiver that allows your system to receive image data that has been pushed from a PACS system. The Dicom Query-Retrieve (DQR) plugin allows XNAT users to connect directly to a PACS or other DICOM Application Entity, using either DIMSE or DICOMweb, send queries to find studies, and import them to their XNAT with custom relabeling applied en route. Users can also send image data from XNAT to the PACS. 
...
* Set up and manage connections to PACS systems and other DICOM AEs, over DIMSE or DICOMweb (QIDO-RS/WADO-RS)
...
Instructions on installing XNAT plugins can be found here: https://wiki.xnat.org/documentation/deploying-plugins-in-xnat