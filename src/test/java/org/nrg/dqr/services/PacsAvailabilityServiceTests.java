...
import org.nrg.dqr.configuration.PacsAvailabilityServiceTestsConfig;
...
@SpringJUnitJupiterConfig(PacsAvailabilityServiceTests.class)
@ContextConfiguration(classes = PacsAvailabilityServiceTestsConfig.class)
@DirtiesContext(classMode = DirtiesContext.ClassMode.AFTER_EACH_TEST_METHOD)
@Slf4j
public class PacsAvailabilityServiceTests {
    @Autowired
    public PacsAvailabilityServiceTests(final PacsAvailabilityEntityService service) {
        _service = service;
    }
    ...
    public void testSearchAndAvailabilityFunctions() {
        final Map<Long, List<PacsAvailability>> availabilities = LongStream.range(1, 6).boxed().collect(Collectors.toMap(Function.identity(), pacsId -> IntStream.range(1, 8).mapToObj(value -> _service.create(PacsAvailability.builder().pacsId(pacsId).dayOfWeek(DayOfWeek.of(value)).threads(4).utilizationPercent(100).availabilityStart("00:00").availabilityEnd("00:00").build())).collect(Collectors.toList())));
